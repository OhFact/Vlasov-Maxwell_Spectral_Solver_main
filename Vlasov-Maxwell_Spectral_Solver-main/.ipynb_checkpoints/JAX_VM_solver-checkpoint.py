...
from Examples.Examples_1D import Landau_damping_HF_1D
from Examples.Examples_2D import Kelvin_Helmholtz_2D
...
def generate_Hermite_basis(xi_x, xi_y, xi_z, Nn, Nm, Np, indices):
    ...
    xi_x,y,z - spatial coordinate in any direction
    Nn,m,p - number of modes in any direction
...
def compute_C_nmp(f, alpha, u, Nx, Ny, Nz, Lx, Ly, Lz, Nn, Nm, Np, indices):
    ...
    Given function f,
    vars alpha, u,
    length in all directions L_x, L_y, L_z,
    modes in position space N_x, N_y, N_z, and
    modes in velocity space N_n, N_m, N_p,
    C_{nmp} is computed
    """
...
def compute_dCk_s_dt(Ck, Fk, kx_grid, ky_grid, kz_grid, Lx, Ly, Lz, nu, alpha_s, u_s, qs, Omega_cs, Nn, Nm, Np, indices):
    """
    This function simulates time evolution of the coefficients C
    Ck - hermite fourier coefficients
    Fk - force coefficients
    kx,y,z_grid - grid of fourier transformation variables
    length in all directions L_x, L_y, L_z,
    modes in velocity space N_n, N_m, N_p,
    Indices -
    ...
    qs - normalized charge of each species (normalized to fundamental charge)
    """
...
def ampere_maxwell_current(qs, alpha_s, u_s, Ck, Nn, Nm, Np, Ns):
    ...
    Nn,m,p - modes in velocity space directions
