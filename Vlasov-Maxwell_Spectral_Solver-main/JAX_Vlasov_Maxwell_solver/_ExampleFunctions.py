...
#landau
def Landau_damping_1D(Lx, Omega_cs, mi_me):
    """
    ...
    B = lambda x, y, z: jnp.array([Omega_cs * jnp.ones_like(x), jnp.zeros_like(y), jnp.zeros_like(z)])
...
def Landau_damping_HF_1D(Lx, Ly, Lz, Omega_cs, alpha_s, alpha_i, Nn):
    """
    I have to add docstrings!
    """

    vte = alpha_s / jnp.sqrt(2)  # Electron thermal velocity.
    ...
    #    Fk_0 = Fk_0.at[3, 0, 1, 0].set(Omega_cs)
